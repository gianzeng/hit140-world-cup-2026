...
def derive(d):
    return d.groupby("team").agg(
        opponent_shots_total=("opponent_shots", "sum"),
        opponent_shots_per_match=("opponent_shots", "mean"),
        opponent_sot_per_match=("opponent_sot", "mean"),
    )


def add_save_ratio(context):
    assert (context.opponent_shots_total > 0).all()
    context["saves_per_100_opponent_shots"] = (
        100 * context.saves / context.opponent_shots_total
    )
    return context
...
def run(context=None):
    ...
    plot_rates(
        context,
        "saves_per_100_opponent_shots",
        "Saves relative to all opposition attempts",
        "Saves per 100 opponent shots (not save %)",
        "saves_exposure",
    )
    return summarise(
        context,
