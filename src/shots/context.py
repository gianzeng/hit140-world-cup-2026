...
def derive(d):
    d = d.assign(shot_share=d.shots / (d.shots + d.opponent_shots))
    return d.groupby("team").agg(mean_match_shot_share=("shot_share", "mean"))
...
def run(context=None):
    ...
    plot_rates(
        context,
        "mean_match_shot_share",
        "Share of match shooting activity",
        "Team mean of match shot share",
        "shots_share",
    )
    return summarise(context, "03_shots", ["mean_match_shot_share"])
